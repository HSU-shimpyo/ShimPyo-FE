...
export default function MonthlyPefAvg() {
  ...
  useEffect(() => {
    getMonthlyAverage().then((res) => {
      ...
      setWeek1(res.week1Average || 40); // 값이 없을 경우 기본값 0
      setWeek2(res.week2Average || 40);
      setWeek3(res.week3Average || 40);
      setWeek4(res.week4Average || 40);
    });
  }, []);

  // 데이터가 설정되었을 때만 barData 생성
  const barData = [
    { value: week1, label: '1주차', frontColor: (week1!==40) ? '#9CC9F5' : '#F1F1F5'},
    { value: week2, label: '2주차', frontColor: (week2!==40) ? '#9CC9F5' : '#F1F1F5' },
    { value: week3, label: '3주차', frontColor: (week3!==40) ? '#9CC9F5' : '#F1F1F5' },
    { value: week4, label: '4주차', frontColor: (week4!==40) ? '#9CC9F5' : '#F1F1F5' }
  ];
